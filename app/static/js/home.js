...
async function askAI() {
    ...
    const selectedLLM = document.getElementById('llmSelect').value; // Get selected LLM
    ...
    try {
        const result = await apiRequest('/ai', {
            ...
            body: JSON.stringify({ query, llm: selectedLLM }), // Include selected LLM
        });
...
async function askPDF() {
    ...
    try {
        ...
        const result = await apiRequest('/ask_pdf', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ query, promptType, llm: selectedLLM }), // Include selected LLM
        });
